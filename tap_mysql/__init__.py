...
def do_discover(mysql_conn, config):
    catalog = discover_catalog(mysql_conn, config.get('filter_dbs'))
    json.dump(catalog.to_dict(), sys.stdout, indent=2)


def log_engine(mysql_conn, catalog_entry: singer.CatalogEntry):
    is_view = catalog_entry.metadata.root.is_view
    ...
    if is_view:
        ...
        with connect_with_backoff(mysql_conn) as open_conn:
            with open_conn.cursor() as cur:
                ...
                if row:
                    LOGGER.info('Beginning sync for %s table %s.%s', row[0], database_name, catalog_entry.table)


def is_valid_currently_syncing_stream(selected_stream, state):
    replication_method = selected_stream.metadata.root.replication_method
...
def get_non_binlog_streams(mysql_conn, catalog: singer.Catalog, config, state):
    """
    ...
    selected_streams = [stream for stream in catalog.streams if stream.metadata.root.selected]
    streams_with_state = []
    streams_without_state = []

    for stream in selected_streams:
        replication_method = stream.metadata.root.replication_method
        ...
        if not stream_state:
            ...
            is_view = stream.metadata.root.is_view
...
def get_binlog_streams(mysql_conn, catalog: singer.Catalog, config, state):
    discovered = discover_catalog(mysql_conn, config.get('filter_dbs'))
    selected_streams = [stream for stream in catalog.streams if stream.metadata.root.selected]
    binlog_streams = []
...
def do_sync_incremental(mysql_conn, catalog_entry, state, columns, batch_config=None):
    ...
    incremental.sync_table(mysql_conn, catalog_entry, state, columns, batch_config=batch_config)

    stream_utils.write_message(singer.StateMessage(value=copy.deepcopy(state)))


# pylint: disable=too-many-arguments
def do_sync_historical_binlog(
    mysql_conn,
    ...
    if max_pk_values and ((use_gtid and gtid) or (log_file and log_pos)):
        ...
        full_table.sync_table(mysql_conn, catalog_entry, state, columns, stream_version, batch_config=batch_config)
    else:
...
def sync_non_binlog_streams(mysql_conn, non_binlog_catalog, state, use_gtid, engine, batch_config=None):
    for catalog_entry in non_binlog_catalog.streams:
        ...
        replication_method = catalog_entry.metadata.root.replication_method

        database_name = common.get_database_name(catalog_entry)

        with metrics.job_timer(job_type='sync_table') as timer:
            ...
            if replication_method == 'INCREMENTAL':
                do_sync_incremental(mysql_conn, catalog_entry, state, columns, batch_config=batch_config)
            elif replication_method == 'LOG_BASED':
                do_sync_historical_binlog(
                    mysql_conn,
                    catalog_entry,
                    state,
                    columns,
                ...
                do_sync_full_table(mysql_conn, catalog_entry, state, columns, batch_config=batch_config)
            else:
...
def sync_binlog_streams(mysql_conn, binlog_catalog, config, state):
    ...
    if binlog_catalog.streams:
        ...
        with metrics.job_timer(job_type='sync_binlog'):
