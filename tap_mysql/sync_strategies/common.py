...
import orjson
import singer
from singer import CatalogEntry, Metadata, RecordMessage, metrics, utils

from tap_mysql import stream_utils
from tap_mysql.stream_utils import get_key_properties, orjson_default
...
def generate_tap_stream_id(table_schema: str, table_name: str) -> str:
...
def get_database_name(catalog_entry: CatalogEntry) -> str | None:
    return catalog_entry.metadata.root.database_name
    ...
    *compression* are supported (and, for ``format='arrow'``, that Arrow/ADBC support is
    ...
    def __post_init__(self):
        ...
        if self.compression not in BATCH_COMPRESSIONS:
            raise ValueError(
                f'batch_config.encoding.compression must be one of {sorted(BATCH_COMPRESSIONS, key=str)}, '
...
class BatchWriter:
    ...
    def flush(self):
        ...
        LOGGER.info('Wrote batch file: %s (%d rows, %d bytes)', self._path, self._row_count, size_bytes)
        msg = {
            'type': 'BATCH',
            'stream': self.stream_name,
            'encoding': {'format': 'jsonl', 'compression': 'gzip' if self._batch_config.gzip_compressed else None},
...
def _boolean_columns(catalog_entry) -> set:
    ...
    return {
        name
        for name, prop in catalog_entry.schema.properties.items()
            ...
            for record_batch in reader:
                ...
                if replication_method in {'FULL_TABLE', 'LOG_BASED'}:
                    ...
                    if max_pk_values and key_properties:
                        last_pk_fetched = {pk: record_batch.column(pk)[last_row_idx].as_py() for pk in key_properties}
