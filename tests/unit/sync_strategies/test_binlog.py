...
from singer import Catalog, CatalogEntry, MetadataMapping, RecordMessage, Schema, SchemaMessage, StateMessage
...
class TestBinlogSyncStrategy:
    def test_add_automatic_properties(self):
        catalog = CatalogEntry(
            tap_stream_id='db-stream', schema=Schema(properties={'x': Schema(type='int')}), metadata=MetadataMapping()
        )
        ...
        assert catalog.schema.properties is not None
        assert 2 == len(catalog.schema.properties)
        assert catalog.schema.properties['x'].type == 'int'

        assert catalog.schema.properties[binlog.SDC_DELETED_AT].type == ['null', 'string']
        assert catalog.schema.properties[binlog.SDC_DELETED_AT].format == 'date-time'

        assert ['x', binlog.SDC_DELETED_AT] == columns

    @patch('tap_mysql.sync_strategies.binlog.calculate_bookmark', return_value=('binlog0001', 50))
    @patch('tap_mysql.sync_strategies.binlog.fetch_current_log_file_and_pos', return_value=('binlog0003', 1000))
    @patch(
        'tap_mysql.sync_strategies.binlog.utils.now',
        return_value=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
    )
    @patch('tap_mysql.sync_strategies.binlog.discover_catalog')
    @patch('tap_mysql.sync_strategies.binlog.make_connection_wrapper')
    def test_sync_binlog_stream_with_log_file_and_pos(self, make_connection_wrapper_mock, discover_catalog_mock, *args):

        # we're dealing with local datetimes, so tests passing depend on the local timezone
        # pin the TZ to EET to avoid flakiness
        os.environ['TZ'] = 'EET'

        config = {'server_id': '123', 'use_gtid': False, 'engine': connection.MYSQL_ENGINE}
        ...
        catalog = {
            'my_db-stream1': {
                'catalog_entry': CatalogEntry(
                    ...
                    schema=Schema(
                        properties={
                            'c_int': Schema(type=['null', 'integer']),
                            'c_varchar': Schema(type=['null', 'string']),
                            'c_timestamp': Schema(type=['null', 'string'], format='date-time'),
                        }
                    ),
            ...
            'my_db-stream2': {
                'catalog_entry': CatalogEntry(
                    ...
                    schema=Schema(
                        properties={
                            'c_bool': Schema(type=['null', 'bool']),
                            'c_double': Schema(type=['null', 'number']),
                            'c_time': Schema(type=['null', 'string'], format='time'),
                        }
                    ),
                    metadata=MetadataMapping.from_iterable(
                        [
        ...
        state = {'bookmarks': {'my_db-stream1': {'version': 1}, 'my_db-stream2': {'version': 1}}}

        with patch('tap_mysql.stream_utils.write_message') as write_msg:
            write_msg.side_effect = lambda msg: singer_messages.append(msg)

            with patch('tap_mysql.sync_strategies.binlog.BinLogStreamReader', autospec=True) as reader_mock:
                ...
                def iter_mock(_):
                    ...
                    log_positions = [50, 300, 4, 100, 250, 7, 20, 140, 300, 470, 999]
                    ...
                    for idx, x in enumerate(
                        [
                            ...
                            get_binlogevent(RotateEvent, {'next_binlog': 'binlog0002', 'position': 4}),
                            get_binlogevent(
                                UpdateRowsEvent,
                                {
                                    'schema': 'my_db',
                            ...
                            get_binlogevent(DeleteRowsEvent, {'schema': 'my_db', 'table': 'stream3', 'columns': []}),
                            get_binlogevent(RotateEvent, {'next_binlog': 'binlog0003', 'position': 7}),
                            get_binlogevent(
                                WriteRowsEvent,
                                {
                                    'schema': 'my_db',
                            ...
                            get_binlogevent(
                                WriteRowsEvent,
                                {
                                    ...
                                    'rows': [
                                        {'values': {'c_int': 3, 'c_varchar': 'varchar 3', 'c_blob': b'dfhdfhsdhf'}},
                                        {'values': {'c_int': 4, 'c_varchar': 'varchar 4', 'c_blob': b'32fgdf243'}},
                                    ],
                                },
                            ),
                            get_binlogevent(
                                DeleteRowsEvent,
                                {
                                    ...
                                    'timestamp': datetime.datetime.timestamp(
                                        datetime.datetime(2021, 1, 1, 10, 20, 55, tzinfo=datetime.timezone.utc)
                                    ),
                                    'columns': [
                                        Column('c_int', FIELD_TYPE.INT24),
                                        Column('c_varchar', FIELD_TYPE.VARCHAR),
                            ...
                            get_binlogevent(
                                DeleteRowsEvent,
                                {
                                    ...
                                    'table': 'stream1',
                                    'timestamp': datetime.datetime.timestamp(
                                        datetime.datetime(2021, 1, 1, 20, 0, 0, tzinfo=datetime.timezone.utc)
                                    ),
                                    'columns': [
                                        Column('c_int', FIELD_TYPE.INT24),
                                        Column('c_varchar', FIELD_TYPE.VARCHAR),
                            ...
                            CatalogEntry(
                                ...
                                schema=Schema(
                                    properties={
                                        'c_int': Schema(type=['null', 'integer']),
                                        'c_varchar': Schema(type=['null', 'string']),
                                        'c_datetime': Schema(type=['null', 'string'], format='date-time'),
                                    }
                                ),
                                metadata=MetadataMapping.from_iterable(
                                    [
                            ...
                            CatalogEntry(
                                ...
                                schema=Schema(
                                    properties={
                                        ...
                                        'c_double': Schema(type=['null', 'number']),
                                        'c_time': Schema(type=['null', 'string'], format='time'),
                                        'c_json': Schema(type=['null', 'string']),
                                    }
                                ),
                                metadata=MetadataMapping.from_iterable(
                                    [
                ...
                binlog.sync_binlog_stream(mysql_con, config, catalog, state)

                discover_catalog_mock.assert_has_calls(
                    [call(mysql_con, None, 'stream1'), call(mysql_con, None, 'stream2')], any_order=False
                )

                assert [type(msg) for msg in singer_messages] == [
                    RecordMessage,
                    RecordMessage,
                    RecordMessage,
                    RecordMessage,
                ...
                assert [msg for msg in singer_messages if isinstance(msg, RecordMessage)] == [
                    RecordMessage(
                        stream='my_db-stream2',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_bool': True, 'c_time': datetime.time(20, 1, 14), 'c_double': 19.44},
                    ),
                    RecordMessage(
                        stream='my_db-stream2',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_bool': False, 'c_time': datetime.time(9, 10, 24), 'c_double': 0.54},
                    ),
                    RecordMessage(
                        stream='my_db-stream2',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_bool': False, 'c_time': datetime.time(8, 13, 12), 'c_double': 100.22},
                    ),
                    RecordMessage(
                        stream='my_db-stream2',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_bool': False, 'c_time': datetime.time(0, 10, 59, 44), 'c_double': 0.54344},
                    ),
                    RecordMessage(
                        stream='my_db-stream2',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_bool': False, 'c_time': datetime.time(0, 0, 0, 38), 'c_double': 1.565667},
                    ),
                    RecordMessage(
                        stream='my_db-stream1',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_int': 1, 'c_timestamp': '2021-03-24T10:12:56+00:00', 'c_varchar': 'varchar 1'},
                    ),
                    RecordMessage(
                        stream='my_db-stream1',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_int': 2, 'c_timestamp': '2019-12-24T03:01:06+00:00', 'c_varchar': 'varchar 2'},
                    ),
                    RecordMessage(
                        stream='my_db-stream2',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_bool': False, 'c_time': datetime.time(0, 0, 0, 38), 'c_double': 10000.234},
                    ),
                    RecordMessage(
                        stream='my_db-stream1',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_int': 3, 'c_varchar': 'varchar 3'},
                    ),
                    RecordMessage(
                        stream='my_db-stream1',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_int': 4, 'c_varchar': 'varchar 4'},
                    ),
                    RecordMessage(
                        stream='my_db-stream1',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                    ...
                    RecordMessage(
                        ...
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                        version=1,
                        record={'c_bool': True, 'c_time': None, 'c_double': -457.10, 'c_json': 'null'},
                    ),
                    RecordMessage(
                        stream='my_db-stream1',
                        time_extracted=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
                ...
                assert [msg for msg in singer_messages if isinstance(msg, StateMessage)] == [
                    StateMessage(
                        value={
                            'bookmarks': {
                                'my_db-stream1': {'log_file': 'binlog0003', 'log_pos': 1000, 'version': 1},
                                'my_db-stream2': {'log_file': 'binlog0003', 'log_pos': 1000, 'version': 1},
                            }
                        }
                    )
                ]
                ...
                assert 1 == reader_mock.return_value.close.call_count

    @patch('tap_mysql.sync_strategies.binlog.calculate_gtid_bookmark', return_value='0-123-555')
    @patch('tap_mysql.sync_strategies.binlog.fetch_current_log_file_and_pos', return_value=('binlog0003', 1000))
    @patch(
        'tap_mysql.sync_strategies.binlog.utils.now',
        return_value=datetime.datetime(2020, 10, 13, 8, 29, 58, tzinfo=datetime.timezone.utc),
    ...
    @patch('tap_mysql.sync_strategies.binlog.discover_catalog')
    @patch('tap_mysql.sync_strategies.binlog.make_connection_wrapper')
    def test_sync_binlog_stream_with_gtid(self, make_connection_wrapper_mock, discover_catalog_mock, *args):

        # we're dealing with local datetimes, so tests passing depend on the local timezone
        # pin the TZ to EET to avoid flakiness
        os.environ['TZ'] = 'EET'

        config = {'server_id': '123', 'use_gtid': True, 'engine': 'mariadb'}
        mysql_con = Mock(spec_set=MySQLConnection)
        ...
        with patch('tap_mysql.stream_utils.write_message') as write_msg:
            ...
            with patch('tap_mysql.sync_strategies.binlog.BinLogStreamReader', autospec=True) as reader_mock:
                ...
                def iter_mock(_):
                    ...
                    log_positions = [50, 300, 520, 4, 100, 250, 7, 14, 20, 140, 300, 470, 999]
                    ...
                    for idx, x in enumerate(
                        [
                            ...
                            get_binlogevent(MariadbGtidEvent, {'gtid': '0-123-556'}),
                            get_binlogevent(
                                UpdateRowsEvent,
                                {
                                    'schema': 'my_db',
                            ...
                            get_binlogevent(MariadbGtidEvent, {'gtid': '0-123-557'}),
                            get_binlogevent(
                                WriteRowsEvent,
                                {
                                    'schema': 'my_db',
                            ...
                            get_binlogevent(MariadbGtidEvent, {'gtid': '0-123-558'}),
                        ]
                    ):
                        reader_mock.return_value.log_file = log_files[idx]
                        reader_mock.return_value.log_pos = log_positions[idx]
    ...
    def test_verify_binlog_config_success(self, connect_with_backoff):
        ...
        cur_mock.__enter__.return_value.fetchone.side_effect = [['ROW'], ['FULL']]
        ...
        cur_mock.__enter__.return_value.execute.assert_has_calls(
            [call('SELECT  @@binlog_format'), call('SELECT  @@binlog_row_image')]
        )
    ...
    def test_verify_binlog_config_fail_if_not_FULL(self, connect_with_backoff):
        ...
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['ROW'], ['Not-FULL']]
        ...
        assert "Unable to replicate binlog stream because binlog_row_image is not set to 'FULL': Not-FULL." == str(
            context.value
        )
    ...
    def test_verify_binlog_config_fail_if_not_ROW(self, connect_with_backoff):
        ...
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['Not-ROW']]
        ...
        assert "Unable to replicate binlog stream because binlog_format is not set to 'ROW': Not-ROW." == str(
            context.value
        )

        connect_with_backoff.assert_called_with(mysql_con)
        cur_mock.__enter__.return_value.execute.assert_has_calls([call('SELECT  @@binlog_format')])
    ...
    def test_verify_binlog_config_fail_if_binlog_row_image_not_supported(self, connect_with_backoff):
        ...
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['ROW']]

        cur_mock.__enter__.return_value.execute.side_effect = [None, InternalError(errno=1193)]
    ...
    def test_verify_gtid_config_success(self, connect_with_backoff):
        ...
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['ON']]
        ...
        connect_with_backoff.assert_called_with(mysql_con)
        cur_mock.__enter__.return_value.execute.assert_has_calls([call('select @@gtid_mode;')])
    ...
    def test_verify_gtid_config_fail_if_not_on(self, connect_with_backoff):
        ...
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['OFF']]
        ...
        assert 'Unable to replicate binlog stream because GTID mode is not enabled.' == str(context.value)
    ...
    def test_fetch_current_log_file_and_pos_success(self, connect_with_backoff):
        mysql_con = MagicMock(spec_set=MySQLConnection).return_value
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['binlog.000033', 345, '']]
        ...
        assert result == ('binlog.000033', 345)

        connect_with_backoff.assert_called_with(mysql_con)
        cur_mock.__enter__.return_value.execute.assert_has_calls([call('SHOW MASTER STATUS')])

    @patch('tap_mysql.sync_strategies.binlog.connect_with_backoff')
    def test_fetch_current_log_file_and_pos_fail_if_no_result(self, connect_with_backoff):
        mysql_con = MagicMock(spec_set=MySQLConnection).return_value
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [None]
        ...
        assert 'MySQL binary logging is not enabled.' == str(context.value)
    ...
    @patch('tap_mysql.sync_strategies.binlog.connect_with_backoff')
    def test_fetch_current_gtid_pos_for_mysql_not_found_expect_exception(self, connect_with_backoff, fetch_server_uuid):
        mysql_con = MagicMock(spec_set=MySQLConnection).return_value
        cur_mock = MagicMock().return_value
        ...
        cur_mock.__enter__.return_value.execute.assert_has_calls([call('select @@GLOBAL.gtid_executed;')])

    @patch('tap_mysql.sync_strategies.binlog.connection.fetch_server_uuid')
    @patch('tap_mysql.sync_strategies.binlog.connect_with_backoff')
    def test_fetch_current_gtid_pos_for_mysql_succeeds(self, connect_with_backoff, fetch_server_uuid):
        ...
        assert '3E11FA47-71CA-11E1-9E33-C80AA9429562:1' == result
    ...
    def test_fetch_current_gtid_pos_for_mariadb_no_gtid_found_expect_exception(
        ...
        assert 'GTID is not present on this server!' in str(context.value)

        connect_with_backoff.assert_called_with(mysql_con)
        fetch_server_id.assert_called_with(mysql_con)

        cur_mock.__enter__.return_value.execute.assert_has_calls([call('select @@gtid_current_pos;')])
    ...
    def test_fetch_current_gtid_pos_no_gtid_found_for_given_server_expect_exception(
        ...
        mysql_con = MagicMock(spec_set=MySQLConnection).return_value
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['0, 0-4-222,']]
        ...
        assert 'No suitable GTID was found for server' in str(context.value)
    ...
    def test_calculate_gtid_bookmark_for_mariadb_returns_earliest(self):
        ...
        binlog_streams = {'stream1': {'schema': {}}, 'stream2': {'schema': {}}, 'stream3': {'schema': {}}}
    ...
    def test_calculate_gtid_bookmark_for_mariadb_no_gtid_found_would_infer_from_binlog(self, connect_with_backoff):
        ...
        mysql_con = MagicMock(spec_set=MySQLConnection).return_value
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['0-4-222'], [4]]
        ...
        cur_mock.__enter__.return_value.execute.assert_has_calls(
            [call('SHOW BINARY LOGS'), call("select BINLOG_GTID_POS('binlog.032', 14);"), call('SELECT @@server_id')]
        )

        assert result == '0-4-222'

    @patch('tap_mysql.sync_strategies.binlog.connect_with_backoff')
    def test_calculate_gtid_bookmark_for_mariadb_no_gtid_found_would_infer_from_binlog_returns_many_gtids(
        ...
        mysql_con = MagicMock(spec_set=MySQLConnection).return_value
        cur_mock = MagicMock().return_value
        cur_mock.__enter__.return_value.fetchone.side_effect = [['0-4-222,,3-4,5-66-2213,6-89-7222'], [89]]
        cur_mock.__enter__.return_value.fetchall.return_value = [
            ('binlog.030',),
            ('binlog.031',),
            ('binlog.032',),
        ...
        assert result == '6-89-7222'

    @patch('tap_mysql.sync_strategies.binlog.calculate_bookmark')
    @patch('tap_mysql.sync_strategies.binlog.connect_with_backoff')
    def test_calculate_gtid_bookmark_for_mariadb_no_gtid_nor_binlog_found_expect_exception(
        self, connect_with_backoff, calculate_bookmark
    ):
        ...
        assert 'No binlog coordinates in state to infer gtid position! Cannot resume logical replication' == str(
            context.value
        )
    ...
    def test_calculate_gtid_bookmark_for_mysql_returns_earliest(self):
        ...
        assert result == '3E11FA47-71CA-11E1-9E33-C80AA9429562:1-2'
    ...
    def test_calculate_gtid_bookmark_for_mysql_no_gtid_found_expect_exception(self):
        ...
        assert "Couldn't find any gtid in state bookmarks to resume logical replication" == str(context.value)
