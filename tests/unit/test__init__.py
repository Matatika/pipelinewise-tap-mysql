from singer import CatalogEntry, MetadataMapping, Schema
...
class TestTapMysql:
    def test_binlog_stream_requires_historical_with_log_coordinates_returns_false(self):
        catalog = CatalogEntry(tap_stream_id='stream_1', schema=Schema(), metadata=MetadataMapping())

        state = {'bookmarks': {'stream_1': {'log_file': 'binlog.0001', 'log_pos': 1123}, 'stream_2': {}}}

        assert not binlog_stream_requires_historical(catalog, state)

    def test_binlog_stream_requires_historical_with_partial_log_coordinates_returns_true(self):
        catalog = CatalogEntry(tap_stream_id='stream_1', schema=Schema(), metadata=MetadataMapping())

        state = {'bookmarks': {'stream_1': {'log_pos': 1123}, 'stream_2': {}}}

        assert binlog_stream_requires_historical(catalog, state)

    def test_binlog_stream_requires_historical_with_gtid_returns_false(self):
        catalog = CatalogEntry(tap_stream_id='stream_1', schema=Schema(), metadata=MetadataMapping())

        state = {'bookmarks': {'stream_1': {'gtid': '0-3834-222'}, 'stream_2': {}}}
    ...
    def test_binlog_stream_requires_historical_with_no_log_coordinates_returns_true(self):
        ...
        state = {'bookmarks': {'stream_1': {}, 'stream_2': {}}}
