...
| json_as_type      | string ('object' or 'string') | False    | 'object'                                                                                                                                                          | Schema type emitted for `json` columns. Column values are always streamed as raw JSON text; `object` matches historical behaviour, `string` also fits non-object JSON documents (arrays, scalars, `null`) |
